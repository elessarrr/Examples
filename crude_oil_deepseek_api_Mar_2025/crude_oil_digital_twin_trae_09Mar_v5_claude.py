...
def main():
    ...
    # Add BBL explanation
    st.info("**Note:** BBL stands for 'Barrel', the standard unit of measurement for petroleum products. One barrel equals 42 U.S. gallons or 159 liters.")
    ...
    if selected_padds and not sim_results.empty and 'Historical' in sim_results['type'].values and 'Simulated' in sim_results['type'].values:
        try:
            ...
            st.markdown(f"""
            ### Projected Impact:
            - Starting Inventory: {start_inventory:,.0f} Million BBL (Barrels)
