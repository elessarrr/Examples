...
def main():
    ...
    # Focus visualization on recent + future data
    ...
    fig.add_vline(
        x=data["analysis_data"]['period'].max(), 
        line_width=2, 
        line_dash="dash", 
        line_color="green",
    ...
    fig.add_trace(go.Scatter(
        x=[data["analysis_data"]['period'].max()],
        y=[data["analysis_data"]['inventory'].iloc[-1]],
        mode='markers',
    ...
    # After the simulation controls but before running simulation
# Add this inside the main() function
    ...
    with scenario_cols[3]:
        if st.button("Reset Defaults"):
            # This will trigger page refresh with defaults
            st.experimental_rerun()
    ...
    with col1:
        start_inv = data["analysis_data"]['inventory'].iloc[-1]
