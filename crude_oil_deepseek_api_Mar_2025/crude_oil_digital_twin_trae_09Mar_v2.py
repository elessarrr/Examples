...
def run_simulation(input_df: pd.DataFrame, params: dict) -> pd.DataFrame:
    ...
    try:
        ...
        for i, date in enumerate(future_dates):
            ...
            # Apply production cut impact
            if params['prod_cut']:
                weekly_change *= 1.5  # 50% more inventory decrease
            
            # Apply demand spike impact
            if params['demand_spike']:
                weekly_change *= 1.3  # 30% more inventory decrease
            
            # Add SPR release effect if enabled
            if params['spr_release']:
                weekly_change += 8000  # 8M barrels/week SPR release
