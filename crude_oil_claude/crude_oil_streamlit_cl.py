gauge={
            ...
            # Handle the dollar sign in the f-string by doubling it
            st.markdown(f"""
    ...
    with tab4:
        ...
        if len(pad_fin_pd) > 0:
            ...
            # Fix the f-string by doubling dollar signs and correcting conditional syntax
            st.markdown(f"""
            ...
            **Strategic Recommendations:**
            1. **Inventory Optimization**: {"Reduce inventory by " + f"{abs(inventory_gap):,.0f} thousand barrels to release capital" if inventory_gap > 0 else "Increase inventory by " + f"{abs(inventory_gap):,.0f} thousand barrels to optimize supply security"}
            ...
            3. **Operational Excellence**: {"Focus on reducing days of supply through improved forecasting and scheduling" if current_days_supply > optimal_days_supply+5 else "Maintain current operational parameters which are within optimal range"}
            """)
...
---
### About the Developer
This digital twin application was developed by a senior product manager with expertise in digital transformation and Industry 4.0 solutions. Specializing in transforming raw data into actionable business insights through modern visualization and simulation tools.

**Skills demonstrated:**
- Data engineering and advanced analytics
- Digital twin development and simulation modeling
- Business intelligence and executive reporting
- Financial impact analysis and optimization strategies

**Connect on LinkedIn**: [Your LinkedIn Profile]
""")
dataframe(top_correlations, hide_index=True)
            st.markdown("""
            ...
            with col2:
                ...
                fig.add_trace(
                    go.Indicator(
                        ...
                        gauge={
                            "axis": {"range": [0, 100]},import streamlit as st
import polars as pl
import pandas as pd
import numpy as np
